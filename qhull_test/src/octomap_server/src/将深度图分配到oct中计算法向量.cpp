...
struct comp {
  bool operator()(const octomap::OcTreeKey *p1, const octomap::OcTreeKey *p2) const 
  {
    bool tb=(p1->k[0] == p2->k[0]) && (p1->k[1] == p2->k[1]) && (p1->k[2] == p2->k[2]);
    return !tb;
  }
};
...
{
  int a;int b;int c;
	bool operator==( const OCTkey& out_v) const //需要重载==才能find
	{
		return a == out_v.a && b == out_v.b && c == out_v.c;
	}
};
...
int main (int argc, char** argv)
{
  ...
  // cloud_org1->points.clear();
  // for(float tsx=0;tsx<=2;tsx=tsx+0.01)
  // for(float tsy=0;tsy<=2;tsy=tsy+0.01)
  // {
  // pcl::PointXYZ ts;
  // ts.x=tsx;  ts.y=tsy;  ts.z=1;
  // cloud_org1->push_back(ts);
  // }
  ...
  {
    // if(i%1440==720) i=i+720;//如果进行降采样,可以节约将近一半的时间
    ...
    *thisk=m_octree->coordToKey(cloud_org1->points[i].x,cloud_org1->points[i].y,cloud_org1->points[i].z);    
    ...
    // std::cout<<"now"<<thisk->k[0]<<" "<<thisk->k[1]<<" "<<thisk->k[2]<<std::endl;
    ...
    else
    {
      ...
      // thismap.insert(make_pair(thisk,pvector));      
      thismap[thisk]=pvector;      
      std::cout<<"new"<<thisk->k[0]<<" "<<thisk->k[1]<<" "<<thisk->k[2]<<std::endl;
      // std::cout<<"have a pointcloud"<<std::endl;
    }
  }
  // return 0;
  ...
  std::cout<<"Size"<<cloud_plane->points.size()<<"Time into grid"<<st_time1-st_time<<"Time into plane"<<st_time2-st_time1<<"Total time"<<st_time2-st_time <<std::endl;
  //总共时间150ms
  // std::map<>
